...
public interface PromoRepository extends JpaRepository<Promo, Long> {
    ...
    @Query(value = "select * from flight_agency.promo where ?1 between promo_date_start and promo_date_end",
            nativeQuery = true)
    List<Promo> getPromoRunningNow(LocalDateTime time, Pageable pageable);

    @Query(value = "select * from flight_agency.promo where promo_date_end < ?1",
            nativeQuery = true)
    List<Promo> getPromoExpired(LocalDateTime time, Pageable pageable);

    @Query(value = "select * from flight_agency.promo where promo_date_start > ?1",
            nativeQuery = true)
